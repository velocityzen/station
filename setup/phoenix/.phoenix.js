...
const setLayout = function(layout, window = Window.focused()) {
...
const cycleLayouts = function(...layouts) {
  let i = 0;
...
const handlers = [];
const disableKeys = () => handlers.forEach(key => key.disable());
const enableKeys = () => handlers.forEach(key => key.enable());
const bind = (key, mods, callback, always) => {
